...
//                     p.email && <option key={i} value={i}>{p.name || p.email}</option>
//                   ))}
//                 </select>
//               </div>
...
//                               <div className="ml-2">
//                                 <div className="text-sm font-medium text-gray-900">{participant.name || "No name"}</div>
...
export function CreateSplitBox({ isOpen, onClose }) {
  ...
  useEffect(() => {
    ...
    if (user && formData.participants.length === 0) {
      setFormData(prev => ({
        ...prev,
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        paidById: user.id, // Always use logged-in user as payer
          ...
          {activeTab === "details" ? (
            <div className="space-y-4">
              ...
              <div>
                ...
                >
                  {formData.participants.map((p, i) => (
                    <option key={i} value={i === 0 ? user.id : i}>
                      {i === 0 ? `You (${user.name})` : (p.name || p.email)}
                    </option>
                  ))}
                </select>
              </div>
                            ...
                            <div className="flex items-center">
                              <div className="ml-2">
                                <div className="text-sm font-medium text-gray-900">
                                  {index === 0 ? `You (${user.name})` : (participant.name || "No name")}
                                </div>
                                <div className="text-sm text-gray-500">{participant.email}</div>
                              </div>
                            </div>
                ...
                {splitPreview.length > 0 && (
                  <div className="mt-4 p-4 bg-gray-50 rounded-md">
                    ...
                    <ul className="space-y-1">
                      {formData.participants.map((participant, index) => (
                        <li key={index} className="flex justify-between">
                          <span>{index === 0 ? `You (${user.name})` : (participant.name || participant.email)}:</span>
                          <span>
                            {formData.currency === "INR" ? "₹" : 
                             formData.currency === "USD" ? "$" : "€"}
                            {splitPreview[index]?.toFixed(2) || "0.00"}
